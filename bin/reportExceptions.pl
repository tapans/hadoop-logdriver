...
my $groupByEN = 0; ## By default, do not group by exception names (group by date instead)
...
if (@args < 6) {
  print <<"END"
Usage: reportExceptions [OPTIONS] DC SERVICE COMPONENT START END OUTPUT_DIR [REGEX]
Note:
  If OUTPUT_DIR is '-', then results are written to stdout.
Options:
  -groupByEN	      Group the results by ExceptionName instead of the default grouping by date
  -v                  Verbose output.
...
Example 1: reportExceptions 6 protect '{zeus,hera}' '2 days ago' 'now' -
Example 2: reportExceptions -groupByEN 6 protect '{zeus,hera}' '2 days ago' 'now' -
Example 3: reportExceptions 6 protect '{zeus,hera}' '2 days ago' 'now' - 'com.bronzeage.lib.utils.db.DbException'
END
  ;
  exit 1;
}
...
my $out       = shift @args;
my $regex     = shift @args
		|| 'CAUGHT: \S+'; ##exception log is defined by existence of keyword 'CAUGHT: ' followed by non-whitespace characters
...
if ($foundresults == 0) {
  ...
  $props .= " -param groupByEN=$groupByEN";
  ...
  my $pig_cmd = "pig -Dmapred.job.queue.name=$queue_name -Dpig.additional.jars=$additional_jars $pig_opts $props"
          . " -l $pig_tmp -f $PIG_DIR/reportExceptionsHelper.pg $redirects 1>&2";
  ...
  my $pig_cmd = "HADOOP_CONF_DIR=/dev/null /usr/lib/pig/bin/pig -Dpig.additional.jars=$additional_jars $pig_opts $props"
          . " -l $pig_tmp -x local -f $PIG_DIR/reportExceptionsHelperLocal.pg $redirects 1>&2";
